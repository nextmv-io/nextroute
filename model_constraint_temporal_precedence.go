...
// TemporalPrecedenceConstraint is a constraint that ensures that certain stops
// are visited before or after other stops based on the time. In contrast to
// "normal" precedence constraints, temporal precedence constraints take into
// account the time at which a stop is visited instead of the order WITHIN a
// route.
type TemporalPrecedenceConstraint interface {
	Identifier
	ModelConstraint

	// AddRelation adds a temporal precedence relation between two stops.
	AddRelation(before ModelStop, after ModelStop) error
	// Duration returns the expression used to determine the duration between
	// two stops for the purpose of evaluating temporal precedence.
	Duration() VehicleTypeDurationExpression
}
